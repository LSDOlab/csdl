...
- Not store a `Model` object; i.e. deletion of the `Model` object _must
  be allowed_ after a `Simulator` object is constructed, imposing no
  additional memory overhead when `Simulator.run` is called.
