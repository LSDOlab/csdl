...
class DirectiveEmbedMethodsTable(Directive):
    """
    Directive for embedding a table from an Parameters instance.
    """
    ...
    def run(self, file_dir, file_name, embed_num_indent, args):
        ...
        lines.append(' ' * embed_num_indent + ' ' * 2 +
                     ':widths: 20, 20, 20, 20, 20\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 + ':stub-columns: 0\n')
        lines.append('\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 + '*  -  Method name\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 + '   -  Order\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 + '   -  Num. stages\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 +
                     '   -  Step vec. size\n')
        lines.append(' ' * embed_num_indent + ' ' * 2 + '   -  Type\n')
