...
class Log10Comp(ExplicitComponent):
    '''
    This is a component that computes the logarithm base 10 using numpy.log10()
    ...
    def compute(self, inputs, outputs):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        outputs[out_name] = np.log10(inputs[in_name]).flatten()

    def compute_partials(self, inputs, partials):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        partials[out_name,
                 in_name] = (1.0 / (np.log(10) * (inputs[in_name]))).flatten()
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        'log10',
        Log10Comp(in_name='x', out_name='y', shape=(n, )),
        promotes=['*'],
    )
    prob.setup()
