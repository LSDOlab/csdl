...
class CotanComp(ExplicitComponent):
    '''
    This is a component that computes the cotangent using 1/numpy.tan()
    ...
    def compute(self, inputs, outputs):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        outputs[out_name] = 1.0 / np.tan(inputs[in_name]).flatten()

    def compute_partials(self, inputs, partials):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        partials[out_name,
                 in_name] = -1.0 / (np.sin(inputs[in_name])**2).flatten()
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        'cotan',
        CotanComp(in_name='x', out_name='y', shape=(n, )),
        promotes=['*'],
    )
    prob.setup()
