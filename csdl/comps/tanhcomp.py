...
class TanhComp(ExplicitComponent):
    '''
    This is a component that computes the hyperbolic tangent using numpy.tanh()
    ...
    def compute(self, inputs, outputs):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        outputs[out_name] = np.tanh(inputs[in_name]).flatten()
