...
class ExpComp(ExplicitComponent):
    '''
    This is a component that computes the exponential using numpy.exp()
    ...
    def compute(self, inputs, outputs):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        outputs[out_name] = np.exp(inputs[in_name]).flatten()

    def compute_partials(self, inputs, partials):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        partials[out_name, in_name] = np.exp(inputs[in_name]).flatten()
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        'exp',
        ExpComp(in_name='x', out_name='y', shape=(n, )),
        promotes=['*'],
    )
    prob.setup()
