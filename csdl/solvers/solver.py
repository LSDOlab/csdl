...
class BlockLinearSolver(LinearSolver):
    ...
    def _create_rhs_vecs(self):
        ...
        for vec_name in system._lin_rel_vec_name_list:
            if self._mode == 'fwd':
                rhs[vec_name] = system._vectors['residual'][vec_name].asarray(
                    True)
            else:
                rhs[vec_name] = system._vectors['output'][vec_name].asarray(
                    True)
    ...
    def _update_rhs_vecs(self):
        ...
        for vec_name in system._lin_rel_vec_name_list:
            if self._mode == 'fwd':
                self._rhs_vecs[vec_name][:] = system._vectors['residual'][
                    vec_name].asarray()
            else:
                self._rhs_vecs[vec_name][:] = system._vectors['output'][
                    vec_name].asarray()
    ...
    def _set_complex_step_mode(self, active):
        ...
        for vec_name in self._system()._lin_rel_vec_name_list:
            if active:
                self._rhs_vecs[vec_name] = self._rhs_vecs[vec_name].astype(
                    np.complex)
            else:
