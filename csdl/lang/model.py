...
class Model:
    ...
    def __init__(self, **kwargs):
        Model._count += 1
        self.rep: GraphRepresentation | None = None
        self.defined = False
        self.sorted_nodes: List[Node] = []
        self.subgraphs: List[Subgraph] = []
        self.implicit_operations: List[ImplicitOperation
    ...
    def _bracketed_search(
        ...
        **Returns**
        ...
        """
        out_res_map, res_out_map, out_in_map = self._expose_nonresidual_outputs(
            implicit_model,
            arguments,
            states,
            residuals,
    ...
    def _expose_nonresidual_outputs(
        self,
        implicit_model: 'Model',
        ...
        residuals: List[str],
        expose: List[str] = [],
    ) -> Tuple[dict[str, Output], dict[str, DeclaredVariable | Output],
               dict[str, List[DeclaredVariable]]]:
        ...
        if not isinstance(implicit_model, Model):
            raise TypeError("{} is not a Model".format(implicit_model))

        # NOTE: top level inputs will be unused, so we don't allow them
        if len(implicit_model.inputs) > 0:
            raise ValueError(
        ...
        # register outputs that user wants to expose
        # TODO: expose residuals and add constraints for SURF
        vars = list(
            filter(lambda x: x.name in set(expose),
        ...
        # problem
        implicit_model.design_variables = dict()
        implicit_model.objective = None
        implicit_model.constraints = dict()
        ...
        for residual_name in residuals:
            ...
            for var in implicit_model.registered_outputs:
        ...
        for res in implicit_model.registered_outputs:
            if res.name.startswith(_residual):
                ...
                if isinstance(expose_var, Output):
        ...
        for dv in implicit_model.declared_variables:
