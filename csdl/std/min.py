...
def min(*vars, axis=None, rho=20.):
    ...
    **Parameters**
    ...
    if axis is not None:
        ...
        if len(vars) == 1:
            # min along axis of a single array
            if len(vars[0].shape) > 1:
                # tuple guaranteed to be nonempty when taking min along
        ...
        # axes == None
        if len(vars) == 1:
            # take min of all elements of a single array, regardless of
            # shape
            shape = (1, )
        else:
            # take min of each set of elements with same array indices
